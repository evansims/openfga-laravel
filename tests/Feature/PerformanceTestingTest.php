...
class PerformanceTestingTest extends TestCase
{
    use RefreshDatabase, FakesOpenFga, MeasuresPerformance;
    ...
    public function test_single_permission_check_performance()
    {
        $fake = $this->getFakeOpenFga();
        $fake->grant('user:1', 'editor', 'document:1');

        // Measure a single check
        $metrics = $this->measure('single-check', function () use ($fake) {
            return $fake->check('user:1', 'editor', 'document:1');
        });
    ...
    public function test_permission_check_completes_within_time_limit()
    {
        $fake = $this->getFakeOpenFga();
        $fake->grant('user:1', 'viewer', 'post:1');

        // Assert operation completes within 10ms
        $this->assertCompletesWithin(10, function () use ($fake) {
    ...
    public function test_batch_operations_performance()
    {
        ...
        $results = $this->benchmark('batch-write-100', function () use ($fake) {
            $writes = [];
            for ($i = 1; $i <= 100; $i++) {
                $writes[] = [
                    'user' => "user:{$i}",
    ...
    public function test_comparing_single_vs_batch_checks()
    {
        $fake = $this->getFakeOpenFga();
    ...
    public function test_memory_usage_assertion()
    {
        $fake = $this->getFakeOpenFga();

        // Assert memory usage stays below 1MB
        $this->assertMemoryUsageBelow(1024 * 1024, function () use ($fake) {
    ...
    public function test_performance_scaling_with_data_size()
    {
        $fake = $this->getFakeOpenFga();
        $results = [];
        ...
        foreach ([10, 50, 100, 500, 1000] as $size) {
            ...
            $metrics = $this->measure("check-with-{$size}-permissions", function () use ($fake) {
                return $fake->check('user:1', 'viewer', 'document:1');
            });
        ...
        $this->assertLessThan($results[10] * 2, $results[1000], 
            'Performance degraded more than 2x with 100x more data');
    }

    public function test_assert_faster_than()
    {
        $fake = $this->getFakeOpenFga();
    ...
    public function test_performance_within_baseline()
    {
        $fake = $this->getFakeOpenFga();
        ...
        // Assert that checking with more data is within 50% of baseline
    ...
    public function test_detailed_performance_report()
    {
        ...
        $perf->benchmark('bulk-checks', function () use ($fake) {
            for ($i = 1; $i <= 5; $i++) {

...
final class SyncSpatiePermissionsCommand extends Command
{
    ...
    private function processBatchUserPermissions(Collection $batch): void
    {
        ...
        foreach ($batch as $userPermission) {
            ...
            try {
                ...
                // Handle both object and boolean returns
                /** @var bool $allowed */
                $allowed = is_object($exists) && method_exists($exists, 'allowed') 
                    ? $exists->allowed() 
                    : (bool) $exists;
    ...
    private function displaySummary(): void
    {
        ...
        $this->table(
            ['Metric', 'Count', 'Status'],
            [
                ['Permissions Added', $this->added, $this->added > 0 ? '✅' : '➖'],
                ['Permissions Removed', $this->removed, $this->removed > 0 ? '✅' : '➖'],

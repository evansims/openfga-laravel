...
class AnalyzePermissionsCommand extends Command
{
    ...
    public function handle(OpenFgaManager $manager): int
    {
        ...
        try {
            ...
            if ($this->option('show-paths')) {
                $analysis['paths'] = $this->analyzePermissionPaths($client);
            }

            // Find conflicts
            if ($this->option('find-conflicts')) {
                $analysis['conflicts'] = $this->findPermissionConflicts($client);
            }

            // Optimization suggestions
            if ($this->option('optimize')) {
                $analysis['optimizations'] = $this->suggestOptimizations($client);
            }

            // Default analysis if no specific options
            if (empty($analysis)) {
                $analysis = $this->performDefaultAnalysis($client);
            }
     ...
     */
    private function analyzePermissionPaths($client): array
    {
     ...
     */
    private function findPermissionConflicts($client): array
    {
     ...
     */
    private function suggestOptimizations($client): array
    {
     ...
     */
    private function performDefaultAnalysis($client): array
    {
        return [
            'structure' => [

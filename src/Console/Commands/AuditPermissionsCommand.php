...
class AuditPermissionsCommand extends Command
{
     ...
     */
    private function collectAuditData($client): array
    {
        $data = [
            'summary' => [],
            'permissions' => [],
        ...
        if ($user = $this->option('user')) {
            $data['permissions'] = $this->auditUserPermissions($client, $user);
        ...
        elseif ($object = $this->option('object')) {
            $data['permissions'] = $this->auditObjectPermissions($client, $object);
        ...
        // General audit
            ...
            $data = $this->performGeneralAudit($client);
        }
     ...
     */
    private function auditUserPermissions($client, string $user): array
    {
        $permissions = [];
     ...
     */
    private function auditObjectPermissions($client, string $object): array
    {
        $permissions = [];
     ...
     */
    private function performGeneralAudit($client): array
    {
    ...
    private function analyzePermissions(array $permissions): array
    {
        ...
        $broadPermissions = collect($permissions)
            ->filter(fn($p) => str_ends_with($p['object'] ?? '', ':*'))
            ->count();

        if ($broadPermissions > 0) {
            $warnings[] = "Found {$broadPermissions} wildcard permissions that may be overly broad";
        }

        // Check for duplicate permissions
        $duplicates = collect($permissions)
            ->groupBy(fn($p) => ($p['user'] ?? '') . '-' . ($p['relation'] ?? '') . '-' . ($p['object'] ?? ''))
            ->filter(fn($group) => $group->count() > 1)
            ->count();

        if ($duplicates > 0) {
            $warnings[] = "Found {$duplicates} duplicate permission entries";
        }
    ...
    private function displayResults(array $data): void
    {
        ...
        if (! empty($data['summary'])) {
            ...
            $this->table(
                ['Metric', 'Value'],
                collect($data['summary'])->map(fn($value, $key) => [
